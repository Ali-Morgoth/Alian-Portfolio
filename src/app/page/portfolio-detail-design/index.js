...
//             {translations[language].insideprojects.projectSample}
//             </p>
...
//             >
//                 {translations[language].insideprojects.projectDescriptions}
//             </p>
...
//               ></path>
//             </svg>
//             {translations[language].insideprojects.previousProject}
//           </a>
//         </div>
...
//           >
//             {translations[language].insideprojects.nextProject}
//             <svg
//               xmlns="http://www.w3.org/2000/svg"
//               fill="none"
//               viewBox="0 0 24 24"
...
export default function Index({ id }) {
  ...
  const data = dataArray.find((item, index) => index === JSON.parse(id));
  ...
  return (
    <React.Fragment>
      ...
      <div className="bg-gradient-dark-inverse grid grid-cols-12 relative space-x-0 lg:space-x-8">
        ...
        {/* Mapeo para las imagenes de cada proyecto y arriba está la constante para evitar dos imagenes */}
          <div className="grid grid-cols-1 gap-4 w-full lg:w-[650px] lg:ml-40 mt-10">
            {filteredImages.map((image, index) => (
              <div
                key={index}
                className="relative w-full border-4 border-cyan-400 p-1 rounded-md"
              >
                {" "}
                {/* Borde celeste y padding para el contenedor */}
                <Image
                  src={image}
                  alt={`Additional image ${index + 1}`}
                  layout="responsive" // La imagen se ajusta al tamaño del contenedor
                  width={800} // Dimensiones altas para mantener la resolución
                  height={600} // Dimensiones altas para mantener la resolución
                  className="w-full h-auto object-cover rounded-md" // Asegura que la imagen mantenga sus proporciones y cubra el contenedor
                />
              </div>
            ))}
          </div>
          ...
          {/* Render for geolocation project */}
          {data?.title === "Visitor Statistics Project - Next.js" && (
            <div>
              <div>
                <WorldMap />
              </div>

              <div className="grid md:grid-cols-2 gap-5 md:ml-10">
                <div className="m-4 md:pt-8 lg:pl-20">
                  <Card1>
                    <div className="pb-12">
                      <CountriesList />
                    </div>
        ...
        <div className="col-span-12 lg:col-span-4 lg:px-0 sm:px-20">
          <div className="mt-10 sm:mt-24 w-full lg:max-w-[300px] lg:px-1 px-3 sticky top-36 pb-14">
            ...
            <div className="flex flex-wrap">
              {["skill1", "skill2", "skill3", "skill4", "skill5", "skill6"].map(
                (skill) => (
                  <h1
                    key={skill}
                    className="mr-5 text-[14px] bg-[#63c5f1] lg:bg-[#00bcd4] px-2 py-1 rounded-xl font-sans mb-4 text-white lg:text-[#fff]"
                  >
                    {data?.[skill]}
                  </h1>
                )
            ...
            {/* Render for EP production */}
            {data.title === "EP Alian Andahur - Mongen Ta Molkun" &&
              data.youtubeLink && (
                <div className="flex flex-col items-center sm:items-start mt-4 space-y-4">
                  <a
                    href={data.youtubeLink}
            ...
            {/* Render for EP production */}
            {data.title === "LP musical - Folil Choyun" && data.youtubeLink && (
              <div className="flex flex-col items-center sm:items-start mt-4 space-y-4">
                <a
                  href={data.youtubeLink}
                  target="_blank"
      ...
      {/* botones previousProject y nextProject */}
      <div className="relative flex bg-accent-color h-48">
        <div
          className={`group w-1/2 flex items-center justify-center bg-cover ${hind.className}`}
          style={{ backgroundImage: `url(${dataArray[prev]?.images[0]})` }}
